...
function photoClick(){
  ...
  article.onclick = function(evt){
    ...
    let index = Array.prototype.indexOf.call(article.querySelectorAll("img"), evt.target);
...
function loadPhotos(){
  ...
  newImage.onload = function(){
    ...
    if(window.exifr){
      //let img = newImage;
      window.exifr.parse(newImage, {iptc:true/*, userComment: true*/}).then(function(exif) {
        if(exif.ObjectName != undefined){newImage.title = exif.ObjectName}
        //else{newImage.title = "untitled"}
        if(exif.Caption != undefined) {newImage.alt = exif.Caption;}
      });
    } 
    photosLoaded++;
    loadPhotos();
...
function resizeRefresh(){
  ...
  if ((document.querySelectorAll(".largePhoto").length > 0) || (document.querySelectorAll(".splash_img").length > 0))
  {
    document.querySelector("article").querySelectorAll("img")[slideIndex - 1].classList.add("buttonPress");
    setTimeout(function(){ document.querySelector("article").querySelectorAll("img")[slideIndex - 1].classList.remove("buttonPress"); }, 200);
  }
}
...
function openModal(n) {
  ...
  document.querySelector("article").addEventListener('touchstart', handleTouchStart, false);
  document.querySelector("article").addEventListener('touchmove', handleTouchMove, false);
  
  //change the class of all images from "photo" to "largePhoto"
  let photos = document.querySelector("article").querySelectorAll("img");
  for (let i = 0; i < photos.length; i++){photos[i].className = "largePhoto"}
...
function closeModal() {
  ...
  document.querySelector("article").removeEventListener('touchstart', handleTouchStart, false);
  document.querySelector("article").removeEventListener('touchmove', handleTouchMove, false);

 //Change the class of the images back from "largePhoto" to "photo"
  let photos = document.querySelector("article").querySelectorAll("img");
  for (let i = 0; i < photos.length; i++) {photos[i].className = "photo"}
  ...
  let articlePadding = document.querySelector("nav").offsetTop + document.querySelector("nav").offsetHeight;
...
function showSlides(n) {
  ...
  else if (active.classList.contains("swipeLeft") || active.classList.contains("swipeRight")){
    setTimeout(function(){
  ...
  if (photos[slideIndex-1].title != "undefined"){
    document.querySelector("#photoTitle").innerText = photos[slideIndex-1].title;
  }
  else{
...
function handleTouchMove(evt) {
  ...
  if ( Math.abs( xDiff ) > Math.abs( yDiff ) ) {/*most significant*/
    if ( xDiff > 0 ) {
      ...
      if (evt.target.tagName == "IMG"){
        touchInProgress();  
        document.querySelector("article").querySelectorAll("img")[index].classList.add("swipeLeft");
        plusSlides(-1);
      }
    } else if ( xDiff < 0 )  {
        /* right swipe */
      if (evt.target.tagName == "IMG"){  
        touchInProgress();  
        document.querySelector("article").querySelectorAll("img")[index].classList.add("swipeRight");
        plusSlides(1);  
      }
    }                       
  } else {
